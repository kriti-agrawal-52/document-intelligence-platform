...
from shared.config import get_config, get_env_vars # To get database name and credentials
...
async def connect_to_mongo():
    ...
    _extracted_texts_collection = _mongo_db[config.databases.mongodb.extracted_text_collection]
    ...
    try:
        ...
        print("MongoDB unique index 'unique_image_name_per_user_index' created/checked for Text Extraction Service.")
    except Exception as e:
        print(f"Error creating MongoDB index for Text Extraction: {e}")
        raise

    print(f"Successfully connected to MongoDB database '{mongo_db_name}' and collection '{_extracted_texts_collection.name}' for Text Extraction.")
