...
from text_extraction_service.database import connect_to_mongo, close_mongo_connection, get_extracted_texts_collection
from motor.motor_asyncio import AsyncIOMotorCollection
from pymongo.errors import DuplicateKeyError
from text_extraction_service.redis_cache import connect_to_redis, close_redis_connection, get_cached_extraction, cache_extraction
from shared.auth_utils import get_current_user_id
from shared.jwt_blacklist import init_jwt_blacklist_redis, close_jwt_blacklist_redis
...
S3_BUCKET = env_vars.S3_USER_IMAGES_BUCKET
...
async def create_openai_image_message(image_message: dict, prompt: str):
    ...
    return {"role": "user", "content": [{"type": "text", "text": prompt}, image_message]}

async def cache_user_recent_extractions(user_id: int, document_id: str, image_name: str, s3_url: str):
    """Cache user's recent extractions in Redis for fast retrieval."""
    try:
        cache_key = config_data.cache.redis_key_patterns.user_recent_extractions.format(user_id=user_id)
        extraction_data = {
        ...
        await cache_extraction(cache_key, json.dumps(extraction_data), ttl=config_data.cache.ttl.user_recent_extractions)
...
# --- API Endpoints ---
@app.get(config_data.services.text_extraction_service.base_path + "/health", tags=["Health"])
...
async def extract_text_from_image(
    ...
    existing_doc = await collection.find_one({"image_name": image_name, "user_id": user_id})
    if existing_doc:
        raise HTTPException(
    ...
    try:
        ...
        await collection.update_one(
            ...
            {"$set": {"s3_url": s3_url, "status": "processing_extraction", "updated_at": datetime.now(timezone.utc)}}
        )
        ...
        if not image_message_content:
            ...
            raise HTTPException(status_code=500, detail="Failed to encode image for processing.")

        prompt_text = config_data.openai.prompt_for_text_extraction
        openai_message = await create_openai_image_message(image_message_content, prompt_text)
        ...
        raise HTTPException(status_code=e.status_code or 500, detail=f"OpenAI API error: {e.response.text}")
    except Exception as e:
        # Update document status to failed
        if 'document_id' in locals():
            await collection.update_one(
