...
class TestCompleteDocumentFlow:
    ...
    def test_complete_document_processing_flow(self, api_base_url, authenticated_user, test_image):
        ...
        docs_response = requests.get(
            f"{api_base_url}/extract/documents",
            headers=headers
        )
        ...
        our_doc = next((doc for doc in updated_docs if doc["image_name"] == document_name), None)
        ...
        doc_detail_response = requests.get(
            f"{api_base_url}/extract/document/{document_name}",
            headers=headers
        )
        ...
        paginated_response = requests.get(
            f"{api_base_url}/extract/documents?limit=1&skip=0",
            headers=headers
        )
    ...
    def test_cross_user_document_isolation(self, api_base_url):
        ...
        # Login both users
        user1_token = requests.post(f"{api_base_url}/auth/token", data={
            "username": user1_data["username"],
            "password": user1_data["password"]
        }).json()["access_token"]
        
        user2_token = requests.post(f"{api_base_url}/auth/token", data={
        ...
        doc_response = requests.get(
            f"{api_base_url}/extract/document/user1_document",
            headers=user2_headers
        )
